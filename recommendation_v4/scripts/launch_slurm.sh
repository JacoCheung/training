...
orchestrate() {
  ...
  echo "[$(date)] config: MODE=$MODE START_TS=$START_TS NUM_TRAIN_TS=$NUM_TRAIN_TS INTER_WINDOW_SHUFFLE=${INTER_WINDOW_SHUFFLE:-<unset>} INTER_WINDOW_SHUFFLE_OFFSET=${INTER_WINDOW_SHUFFLE_OFFSET:-<unset>} INTER_WINDOW_SHUFFLE_TOTAL_TS=${INTER_WINDOW_SHUFFLE_TOTAL_TS:-<unset>} SEED=${SEED:-<gin-default>} ENABLE_DROPOUT=${ENABLE_DROPOUT:-<gin-default:1>} ENABLE_DEDUP=${ENABLE_DEDUP:-<gin-default:0>} NUM_TRAIN_BATCHES=$NUM_TRAIN_BATCHES NUM_EVAL_BATCHES=$NUM_EVAL_BATCHES METRIC_LOG_FREQ=$METRIC_LOG_FREQ" | tee -a "$LOG"
  ...
  srun --ntasks-per-node=1 bash -c "
    ...
    docker exec \
      ...
      ${INTER_WINDOW_SHUFFLE:+-e INTER_WINDOW_SHUFFLE=$INTER_WINDOW_SHUFFLE} \
      ${INTER_WINDOW_SHUFFLE_OFFSET:+-e INTER_WINDOW_SHUFFLE_OFFSET=$INTER_WINDOW_SHUFFLE_OFFSET} \
      ${INTER_WINDOW_SHUFFLE_TOTAL_TS:+-e INTER_WINDOW_SHUFFLE_TOTAL_TS=$INTER_WINDOW_SHUFFLE_TOTAL_TS} \
      ...
      ${DIE_AT_STEP:+-e DIE_AT_STEP=$DIE_AT_STEP} \
      ...
      ${ENABLE_DROPOUT:+-e ENABLE_DROPOUT=$ENABLE_DROPOUT} \
      ${ENABLE_DEDUP:+-e ENABLE_DEDUP=$ENABLE_DEDUP} \
      ...
      ${QCOMM_LOWMEM_CODEC:+-e QCOMM_LOWMEM_CODEC=$QCOMM_LOWMEM_CODEC} \
      ${EMB_SHARDING_OVERRIDES:+-e EMB_SHARDING_OVERRIDES=$EMB_SHARDING_OVERRIDES} \
      ${EMB_PLACEMENT_OVERRIDES:+-e EMB_PLACEMENT_OVERRIDES=$EMB_PLACEMENT_OVERRIDES} \
      ${EMB_PLACEMENT:+-e EMB_PLACEMENT=$EMB_PLACEMENT} \
      ${PG_TIMEOUT_S:+-e PG_TIMEOUT_S=$PG_TIMEOUT_S} \
      ${TORCH_NCCL_TRACE_BUFFER_SIZE:+-e TORCH_NCCL_TRACE_BUFFER_SIZE=$TORCH_NCCL_TRACE_BUFFER_SIZE} \
      ${TORCH_NCCL_DUMP_ON_TIMEOUT:+-e TORCH_NCCL_DUMP_ON_TIMEOUT=$TORCH_NCCL_DUMP_ON_TIMEOUT} \
      ${TORCH_NCCL_TRACE_CPP_STACK:+-e TORCH_NCCL_TRACE_CPP_STACK=$TORCH_NCCL_TRACE_CPP_STACK} \
      ${TORCH_NCCL_DEBUG_INFO_TEMP_FILE:+-e TORCH_NCCL_DEBUG_INFO_TEMP_FILE=$TORCH_NCCL_DEBUG_INFO_TEMP_FILE} \
      -e LOG=$LOG \
      $NCCL_ENV_ARGS \
