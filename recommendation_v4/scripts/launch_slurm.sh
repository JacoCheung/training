...
orchestrate() {
  ...
  echo "[$(date)] config: MODE=$MODE START_TS=$START_TS NUM_TRAIN_TS=$NUM_TRAIN_TS INTER_WINDOW_SHUFFLE=${INTER_WINDOW_SHUFFLE:-<unset>} INTER_WINDOW_SHUFFLE_OFFSET=${INTER_WINDOW_SHUFFLE_OFFSET:-<unset>} INTER_WINDOW_SHUFFLE_TOTAL_TS=${INTER_WINDOW_SHUFFLE_TOTAL_TS:-<unset>} SEED=${SEED:-<gin-default>} NUM_TRAIN_BATCHES=$NUM_TRAIN_BATCHES NUM_EVAL_BATCHES=$NUM_EVAL_BATCHES METRIC_LOG_FREQ=$METRIC_LOG_FREQ" | tee -a "$LOG"
