...
INTER_WINDOW_SHUFFLE_ORDER_PATH="${INTER_WINDOW_SHUFFLE_ORDER_PATH:-}"
...
EOS
    ...
    opt_exp EVAL_EVERY_N_WINDOWS "${EVAL_EVERY_N_WINDOWS:-}"
    opt_exp EVAL_EVERY_DATA_PCT "${EVAL_EVERY_DATA_PCT:-}"
    ...
    opt_exp INTER_WINDOW_SHUFFLE_ORDER_PATH "$INTER_WINDOW_SHUFFLE_ORDER_PATH"
    opt_exp NCCL_DMABUF_ENABLE "$NCCL_DMABUF_ENABLE"
    ...
    opt_exp ENABLE_DROPOUT "$ENABLE_DROPOUT"
    opt_exp ENABLE_DEDUP "$ENABLE_DEDUP"
...
pass_envs=(
  ...
  HISTORY_STRATEGY
  SEED DENSE_LR SPARSE_LR MIN_HISTORY HSTU_NUM_LAYERS ENABLE_DROPOUT ENABLE_DEDUP GRAD_CLIP_NORM
)
...
PY
  ...
  env | sort | grep -E '^(CUDA_VISIBLE_DEVICES|GPUS_PER_NODE|NNODES|NODE_RANK|MASTER_ADDR|MASTER_PORT|LOCAL_BATCH_SIZE|BATCH_SIZE|NUM_WORKERS|PREFETCH_FACTOR|PERSISTENT_LOADER|DOUBLE_BUFFER|EVAL_EACH_WINDOW|CKPT_EVERY_N_TS|CKPT_PATH|KEEP_LAST_N|RUN_NAME|TENSORBOARD_LOG_PATH|HSTU_HAMMER_KERNEL|PYTORCH_CUDA_ALLOC_CONF|TORCH_CUDA_ARCH_LIST|TORCH_DIST_TIMEOUT_SECONDS|NCCL_DMABUF_ENABLE|STREAMING_SHUFFLE_FRACTION|STREAMING_SHUFFLE_SEED|INTER_WINDOW_SHUFFLE|INTER_WINDOW_SHUFFLE_OFFSET|INTER_WINDOW_SHUFFLE_TOTAL_TS|INTER_WINDOW_SHUFFLE_ORDER_PATH|HISTORY_STRATEGY|DLRM_DATA_PATH|YAMBDA_CACHE_DIR|PYTHONPATH|HISTORY_LENGTH|MAX_SEQ_LEN|NUM_TRAIN_TS|START_TS|EVAL_HOLDOUT_TS|EVAL_HOLDOUT_NUM_WINDOWS|EVAL_EVERY_N_WINDOWS|EVAL_EVERY_DATA_PCT|NUM_TRAIN_BATCHES|NUM_EVAL_BATCHES|CKPT_TIME_INTERVAL_S|IN_WINDOW_CKPT_FREQ|CKPT_STEP_FREQ|SEED|DENSE_LR|SPARSE_LR|MIN_HISTORY|HSTU_NUM_LAYERS|ENABLE_DROPOUT|ENABLE_DEDUP|GRAD_CLIP_NORM)=' || true
  echo
...
EOS
  ...
  echo "inter_window_shuffle=${INTER_WINDOW_SHUFFLE:-<unset>} inter_window_shuffle_offset=${INTER_WINDOW_SHUFFLE_OFFSET:-<unset>} inter_window_shuffle_total_ts=${INTER_WINDOW_SHUFFLE_TOTAL_TS:-<unset>} inter_window_shuffle_order_path=${INTER_WINDOW_SHUFFLE_ORDER_PATH:-<unset>} seed=${SEED:-<gin-default>}"
  echo "enable_dropout=${ENABLE_DROPOUT:-<gin-default:1>}"
  echo "enable_dedup=${ENABLE_DEDUP:-<gin-default:0>}"
  echo "nccl_dmabuf_enable=${NCCL_DMABUF_ENABLE:-<unset>}"
